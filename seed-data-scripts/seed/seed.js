...
const loadData = async () => {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // publication ID
      const doesPublicationExist = await checkIfPublicationExists(
        title,
        parent1ID,
        parent2ID
      );
      ...
      if (doesPublicationExist) {
        ...
        if (references.length) {
          ...
          const doesPublicationHaveReferences = await checkForRefrences(
            doesPublicationExist.id
          );

          if (doesPublicationHaveReferences.length === 0) {
            formattedReferences = formatReference(
              references,
              doesPublicationExist.id
            );
            ...
            if (formattedReferences.length) {
              await addReferencesToPublication(
                doesPublicationExist.id,
                formattedReferences
              );
      ...
      if (!parent1ID && !parent2ID) {
        ...
        logError(
          "Missing parents for publication: " +
            title +
            "Parents: " +
            parent1 +
            " - " +
      ...
      const didPublicationLaunch = await checkIfPublicationIsLive(
        publicationCreation.id
      );
...
const checkIfPublicationExists = async (title, parent1ID, parent2ID) => {
  ...
  try {
    const publicationsResponse = await axios.get(
      `${url}/publications/seed-data-publications?title=${encodedTitle}`
    );
...
const createPublication = async (title, content) => {
  ...
  try {
    ...
    const create = await axios.post(
      `${url}/publications?apiKey=${apiKey}`,
      payload
    );
    return create.data;
  } catch (err) {
...
const launchPublication = async (id) => {
  try {
    const launched = await axios.put(
      `${url}/publications/${id}/status/LIVE?apiKey=${apiKey}`
    );
...
const formatReference = (references, publicationId) => {
  ...
  for (let i = 0; i < paragraphsArray.length; i++) {
    ...
    let referenceText = newReference.text.replace(/"(.*?)"/g, "$1")
...
const getDOIsFromText = (text) =>
  text.match(/(10\.[0-9a-zA-Z]+\/(?:(?!["&\'])\S)+)\b/g) || [];
