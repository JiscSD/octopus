    ...
    test('Octopus publications pagination', async () => {
        ...
        await Promise.all([
            page.waitForResponse(
                (response) =>
                    response.url().includes('/users/octopus/publication-versions?offset=10&limit=10') && response.ok()
            ),
        ...
        await Promise.all([
            page.waitForResponse(
                (response) =>
                    response.url().includes('/users/octopus/publication-versions?offset=20&limit=10') && response.ok()
            ),
            page.click("'Show More'")
        ]);
