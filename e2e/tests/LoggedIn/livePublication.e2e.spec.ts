...
export const testBookmarking = async (page: Page, id: string) => {
  ...
  const isRemoveBookmarkVisible = await page.isVisible(
    PageModel.livePublication.removeBookmark
  );
  ...
  await expect(page.locator(PageModel.myBookmarks.bookmark)).toHaveAttribute(
    "href",
    `/publications/${id}`
  );
  ...
  await expect(
    page.locator(PageModel.livePublication.removeBookmark)
  ).toBeVisible();
  await page.locator(PageModel.livePublication.removeBookmark).click();
};

export const testFlagging = async (
  page: Page,
  id: string,
  redFlagContent: string
) => {
  await page.goto(`${Helpers.UI_BASE}/publications/${id}`, {
    waitUntil: "domcontentloaded",
  });
  ...
  // Flag publication
  const isRedFlagVisible = await page.isVisible(
    PageModel.livePublication.redFlagLink
  );
  if (isRedFlagVisible) {
    ...
    await page
      .locator(PageModel.livePublication.confirmResolve)
      .locator("visible=true")
      .click();
    await expect(
      page.locator(PageModel.livePublication.redFlagPreview)
    ).not.toBeVisible();
  }
  await expect(
    page.locator(PageModel.livePublication.flagConcern).locator("visible=true")
  ).toBeVisible();
  await page
    .locator(PageModel.livePublication.flagConcern)
    .locator("visible=true")
    .click();
  ...
  await expect(
    page.locator(PageModel.livePublication.redFlagAlert)
  ).toBeVisible();
  await page.locator(PageModel.livePublication.redFlagLink).click();
  await page.locator(PageModel.livePublication.redFlagPreview).click();
...
test.describe("Live Publication", () => {
  ...
  test("Flagging a publication", async ({ browser }) => {
    ...
    testFlagging(
      page,
      "cl3fz14dr0001es6i5ji51rq4",
      "testing the flagging functionality"
    );
  });
