...
describe('Create links', () => {
    ...
    test('User cannot create same link twice', async () => {
        const linkAttemptOne = await testUtils.agent.post('/links').query({
            apiKey: '123456789'
        })
        .send({
            to: 'publication-problem-live',
        ...
        const linkAttemptTwo = await testUtils.agent.post('/links').query({
            apiKey: '123456789'
        })
        .send({
            to: 'publication-problem-live',
...
describe('canLinkBeCreatedBetweenPublicationTypes', () => {
    describe('Should return true', () => {
        ...
        test('REAL_WORLD_APPLICATION -> INTERPRETATION', () => {
            const result = linkService.canLinkBeCreatedBetweenPublicationTypes('REAL_WORLD_APPLICATION', 'INTERPRETATION');
            expect(result).toEqual(true);
        });
