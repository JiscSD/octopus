...
test.describe('Static pages, logged out', () => {
    ...
    test('Check about page', async ({ browser }) => {
        ...
        await expect(page.locator('h1')).toHaveText('Frequently asked questions', { timeout: 10000 });
        await page.goBack();

        await page.locator(PageModel.about.authorGuide).click();
        await expect(page.locator('h1')).toHaveText('Author guide to publishing on Octopus.', { timeout: 10000 });
        await page.goBack();

        await page.locator(PageModel.about.aims).click();
        await expect(page.locator('h1')).toHaveText('Octopus in detail: aims and priorities', { timeout: 10000 });
        await page.goBack();
