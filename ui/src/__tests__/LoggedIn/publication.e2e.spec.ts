...
test.skip('Logged out: Prompted to sign in for actions', async ({ browser, context }) => {
    const page = await browser.newPage({ ignoreHTTPSErrors: true });
    await Helpers.selectFirstPublication(page);
    await expect(page.locator(`text=Sign in for more actions`)).toBeVisible;
});

test.skip('Logged in unverified: Prompted to verify email for actions', async ({ browser, context }) => {
    const page = await browser.newPage({ ignoreHTTPSErrors: true });
    await Helpers.selectFirstPublication(page);
